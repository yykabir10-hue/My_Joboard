...
This workspace is your own repository — there is no upstream to fork or track.

```bash
cd ai-job-search
git init                     # if you have not already
gh repo create <your-name>/german-job-search --private --source=. --remote=origin
git add -A && git commit -m "Initial commit"
git push -u origin main
```

**Create it private.** Once `/setup` runs, `CLAUDE.md` and the profile skill files
hold real personal data, and those files are tracked on purpose so your
personalization is version controlled. The genuinely sensitive files (tracker,
salary data, `documents/`, `seen_jobs.json`, scrape pools) are gitignored either
way, but the profile itself is not.
...
## 8. Keeping your own history

There is no upstream remote to merge from — this repo is self-contained. Commit
your changes normally:
...
Before committing, the same checks CI runs are worth running locally:
